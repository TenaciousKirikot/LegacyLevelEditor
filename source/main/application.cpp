	...
	)
{
	...
	m_current_color_window = m_settings.color_window;
	...
	ColorWindowSettings color_window_settings = ColorWindowSettings();
	loadFromFile("Colors\\" + m_settings.color_window + ".json", color_window_settings_dispatcher, m_theme, color_window_settings);

	m_sound_handler = new SoundHandler(m_theme.sounds, m_settings.sound_volume);
	m_color_window = new ColorWindow(m_strings->operator[]("colorWindowTitle"), m_sound_handler, m_strings, &m_settings, color_window_settings);

	loadFromFile("Layouts\\" + m_settings.layout + ".json", layout_dispatcher, *this, *m_sound_handler, m_settings, m_theme, *m_window, m_color_window, m_layout, m_strings);
...
void Application::loadSettings(vector<String> arguments)
{
	tuple<String, String, String>* load_info = new tuple<String, String, String>[4]
	{
		...
		make_tuple("Colors", arguments[4], m_settings.color_window),
	...
	ComboBox* color_window_size = dynamic_cast<ComboBox*>(m_controls[arguments[10]]);
	if (color_window_size)
	{
		size_t i = 0;
		bool found = false;
		...
		for (; i < 4; i++)
		{
			auto& screen_size = m_color_window_sizes[i];
			if (screen_size == m_color_window_sizes[m_settings.color_window_size > 1 ? 0 : m_settings.color_window_size])
			{
				found = true;
				break;
			}
		...
		color_window_size->forceSetIndex(found ? i : 0);
	}

	CheckBox* color_window_enable_vsync = dynamic_cast<CheckBox*>(m_controls[arguments[11]]);
	if (color_window_enable_vsync)
	{
		color_window_enable_vsync->forceSetChecked(m_settings.color_window_enable_vsync);
	}

	UintNumeric* color_window_fps_limit = dynamic_cast<UintNumeric*>(m_controls[arguments[12]]);
	if (color_window_fps_limit)
	{
		color_window_fps_limit->forceSetValue(m_settings.color_window_fps_limit);
	}
...
void Application::updateIfDirty()
{
	...
	if (m_settings.color_window != m_current_color_window)
	{
		m_settings.color_window = m_current_color_window;
		m_settings_changed = true;
		...
		for (auto& control : m_controls)
		{
			ColorPicker* picker = dynamic_cast<ColorPicker*>(control.second);
			if (picker)
			{
				picker->setColorWindow(m_color_window);
			}
		}
	}
...
void Application::setColorWindowSize(size_t index)
{
	m_color_window->setWindowSize(index);
}

void Application::setColorVsync(bool enable_vsync)
{
	m_settings_changed = true;
	m_color_window->setVsync(enable_vsync);
}

void Application::setColorFramerateLimit(sf::Uint8 fps_limit)
{
	m_color_window->setFramerateLimit(fps_limit);
}
...
void Application::setColorWindow(sf::String control)
{
	ComboBox* combo_box = dynamic_cast<ComboBox*>(m_controls[control]);
	if (combo_box)
	{
		m_current_color_window = combo_box->getItem();
	}
}
...
void Application::close()
{
	if (m_settings_changed)
	{
		...
		writer.write("colorWindowSize", m_settings.color_window_size);
		writer.write("colorWindowEnableVSync", m_settings.color_window_enable_vsync);
		writer.write("colorWindowFpsLimit", m_settings.color_window_fps_limit);
		writer.write("language", toAnsi(m_settings.language));
		writer.write("theme", toAnsi(m_settings.theme));
		writer.write("colorWindow", toAnsi(m_settings.color_window));
		...
		if (loaded)
		{
			...
			std::async(
				...
				[this]()
				{
					if (m_color_window)
					{
						m_color_window->updateVerticesLoop();
						m_color_window->renderLoop();
					}
				}
			);
		}
...
{
	ofstream file = ofstream("output.json"); //JSON opening logic or whatever
	object_writer writer = object_writer(file, writer_configuration().pretty_printing(m_settings.use_pretty_printing).use_tabs(m_settings.use_tabs));
	...
	string game_features_to_unlock = getControlValue<InputControl, std::string>("game_features_to_unlock");
	if (game_features_to_unlock != "")
	{
		string buff = "";
		vector<string> game_features;

		for (auto n : game_features_to_unlock)
		{
			if (n != ' ')
			{
				buff += n;
	...
	write_module(
		...
		[this](object_writer& writer)
		{
			...
			writeTable("PresetPlantList", "preset_plants", writer, new const char* [2]{ "PlantType", "Level" });
		}
	);
	...
	write_module(
		...
		[this](object_writer& writer)
		{
			writeTable("LinkedTiles", "initial_powertiles_list", writer, new const char* [4]{ "Group", "X", "Y", "PropagationDelay" });
		}
	);

	write_module(
		getControlValue<CheckBox, bool>("last_stand"),
		"LastStand",
	...
	for (size_t i = 0; i < waves_count; i++)
	{
		write_module(
			...
			[&, this](object_writer& writer)
			{
				...
				{
					InputControl* input_control = dynamic_cast<InputControl*>(inner_table->getControlAt(i, 0));
					object_writer object_writer = array_writer.nested_object();
					object_writer.write("Type", "RTID(" + input_control->getText().toAnsiString() + "@ZombieTypes)");
					object_writer.close();
				}
	...
	//level_definition_data_writer.write("MusicType", getLevelModule("loot"));


	//level_definition_data_writer.write("description", getControlValue<InputControl, std::string>("description"));
	//level_definition_data_writer.write("description", getControlValue<InputControl, std::string>("description"));
	//level_definition_data_writer.write("description", getControlValue<InputControl, std::string>("description"));
	//level_definition_data_writer.write("description", getControlValue<InputControl, std::string>("description"));
	//level_definition_data_writer.write("description", getControlValue<InputControl, std::string>("description"));
	...
	//sf::String a = getControlValue<InputControl, sf::String>("");
